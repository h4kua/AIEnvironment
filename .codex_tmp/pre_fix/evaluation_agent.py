...
class EvaluationAgent:
    ...
    def run(
        ...
        if final_risk == "WARNING" and has_danger_escalation(
    ...
    def _requires_manual_review(
        ...
        Five independent conditions (any one is sufficient):
          1. Confidence below IRBI-aware threshold (base 0.55, up to 0.70)
          2. Two or more independent failure modes
          3. Baseline alert with SAFE label (two mechanisms disagree)
          4. DANGER output (always requires human confirmation)
          5. DecisionCore uncertainty_score above threshold (high epistemic uncertainty)
